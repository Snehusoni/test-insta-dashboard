...
const SidebarItem = ({ icon, label, onClick, isActive }: SidebarItemProps) => {
	...
	return (
		...
		>
			...
			<span
				className={`text-base ${isActive ? "text-black" : "text-gray-900"} hidden lg:inline`}
			>
				{label}
			</span>
			{!!unreadCount && unreadCount > 0 && label === "Notifications" && (
				<span className="ml-[-18] lg:ml-auto min-w-[20px] h-5 px-2 bg-red-500 text-white text-xs font-semibold flex items-center justify-center rounded-full shadow-sm">
